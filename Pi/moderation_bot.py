...
Handles all moderation commands with proper admin/user logic and replies.
"""
...
warnings_db: Dict[int, Dict[int, list]] = {}  # chat_id -> {user_id: [warnings]}
settings_db: Dict[int, Dict[str, Any]] = {}  # chat_id -> {settings}
rules_db: Dict[int, Dict[str, Any]] = {}  # chat_id -> {rules}

DEFAULT_SETTINGS = {
    "warn_limit": 3,
    "warn_mode": WarningAction.MUTE,
    "warn_mode_duration": None,
    "warn_time": None,  # None = never expire
    "private_rules": False,
}
...
async def get_target_user(
    ...
    if context.args and len(context.args) > 0:
        ...
        if target.startswith("@"):
            ...
            # In groups, the bot can resolve @mentions if privacy mode is off
            # We'll try to find the user from the message's entities
            if message.entities:
                for entity in message.entities:
                    if (
                        entity.type == "mention"
                        and entity.offset < len(message.text)
                    ):
                        mention_text = message.text[
                            entity.offset : entity.offset + entity.length
                        ]
                        if mention_text.lower() == target.lower():
                            # We found the mention but can't get the user object
                            # Return a placeholder that we'll handle
                            pass

            # Try to get user by username via API (requires bot to have privacy mode off)
            try:
                # This won't work in privacy mode, but we try anyway
            ...
            # If we can't resolve @username, we need to handle this case
...
def parse_args_for_user_and_reason(
    args: list, expect_user_at_start: bool = True
) -> tuple:
    """
    Parse command arguments to extract user reference and reason.
    Returns (user_ref, reason, duration)
    """
    if not args:
    ...
    start_idx = 0
    if expect_user_at_start and args[0].startswith("@"):
        user_ref = args[0]
        start_idx = 1

    # Check for duration in remaining args
    for i in range(start_idx, len(args)):
        parsed = parse_duration(args[i])
        if parsed and duration is None:
            duration = parsed
        else:
            reason_parts.append(args[i])

    reason = " ".join(reason_parts) if reason_parts else None
    return user_ref, duration, reason
...
async def mute_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Parse duration and reason from args
    duration = None
    reason = "No reason provided"

    if context.args:
        # Skip @username if present at start
        args_start = 1 if context.args[0].startswith("@") else 0
        remaining_args = context.args[args_start:]

        if remaining_args:
            # Check if first remaining arg is duration
...
async def tmute_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Duration is required for tmute
    duration = None
    reason = "No reason provided"
...
async def ban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if trying to ban someone with equal/higher permissions
    try:
...
async def warns_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Allow users to check their own warnings
    target_user = await get_target_user(update, context)
    if not target_user:
...
def main():
    ...
    application.add_handler(CommandHandler("dwarn", dwarn_command))
    ...
    application.add_handler(CommandHandler("resetallwarns", resetallwarns_command))
    ...
    application.add_handler(CommandHandler("warntime", warntime_command))
    ...
    application.add_handler(CommandHandler("privaterules", privaterules_command))
    ...
    application.run_polling(
        ...
        drop_pending_updates=True,  # Ignore updates before bot started
    )
